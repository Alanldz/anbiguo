...
1. **埋点体系落地**：新增 API-EVT-001 客户端埋点批量上报（表 sys_event_logs，迁移 2026_09_15_100004，append-only）与 API-ADM-106 总后台埋点分析汇总（权限码复用 sys:statistics:view）；契约新增 §十四
2. **注销物理清除闭环**：新增 user:purge-canceled 定时命令（每天 03:30，清除注销满 30 天用户的关联数据；订单依法保留）
3. **总后台安全加固**：新增 admin.ip 白名单中间件（config/admin.php ip_whitelist 配置驱动，默认关闭；支持 CIDR）
4. **客户端补齐**：帮助中心静态页、题库导出（JSON 下载）、分享题库（复制链接）、埋点上报工具接入 4 处关键行为
5. 台账进度：143 个接口已开发 140 个

**影响范围**：server / admin/api / client 三端；向后兼容
...
1. **P1 补齐 7 接口落地**：API-MSG-001~004 消息通知（列表/未读数/单条已读/全部已读）、
   API-MST-001~002 斩题（列表/找回）、API-ERR-001 易错题集（按题库正确率升序聚合）。
2. **新增表与字段**：`user_notifications`（migration `2026_09_15_100002`）；
   `user_wrong_questions` 加列 `right_streak`/`mastered_at`（migration `2026_09_15_100003`）。
3. **斩题机制**：错题连续答对 3 次（MASTER_STREAK 常量，后续可配置化）自动置为已掌握；
   已掌握后再答错自动回到错题本并归零计数。
4. **客户端**：新增消息中心（首页/我的铃铛接入 + 未读角标）、我的斩题（mine 入口）、
   易错题集（题库详情入口）、协议静态页（隐私政策/用户协议，设置页挂接）4 个页面。
5. **台账进度**：140 个接口已开发 137 个（§八同步）。

**影响范围**：server（2 迁移/模型/服务/控制器/路由）、client（4 新页面 + 入口挂接）、docs/04、契约文档 §十一
