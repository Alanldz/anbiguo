...
use App\Http\Middleware\EnsureConsoleUserActiveMiddleware;
use App\Http\Middleware\EnsureUserActiveMiddleware;
use App\Http\Middleware\ForceJsonResponseMiddleware;
use App\Http\Middleware\OperationLogMiddleware;
use App\Http\Middleware\PlatformHeaderMiddleware;
...
return Application::configure(basePath: dirname(__DIR__))
    ->withRouting(
        ...
        then: function () {
            // -------------------------------------------------------------
            // 双域路由（docs/04-API接口规范与登记表.md §一）
            //   客户端    /api/v1/*         守卫 auth:client
            //   用户后台  /console-api/v1/* 守卫 auth:console
            //   总后台是【独立应用】，见 admin/api/，绝不在此注册
            // -------------------------------------------------------------
            ...
            Route::middleware('console')
                ...
                ->name('console.')
                ->group(base_path('routes/console_api.php'));
        },
    )
    ->withMiddleware(function (Middleware $middleware) {
        ...
        $middleware->append(PlatformHeaderMiddleware::class);

        // 路由中间件别名
        // 说明：鉴权直接用框架内置的 auth 中间件带守卫参数，即 `auth:client` / `auth:console`
        //       守卫定义见 config/auth.php，驱动为自研 JwtGuard（app/Auth/JwtGuard.php）
        $middleware->alias([
            'request.id'   => RequestIdMiddleware::class,
            'platform'     => PlatformHeaderMiddleware::class,
            'user.active'  => EnsureUserActiveMiddleware::class,
            'console.user' => EnsureConsoleUserActiveMiddleware::class,
    ...
    ->withExceptions(function (Exceptions $exceptions) {
        ...
        // -------------------------------------------------------------
        $isApi = fn (Request $request) => $request->is('api/*', 'console-api/*');
        ...
        $exceptions->render(function (ValidationException $e, Request $request) use ($isApi) {
            ...
            return ApiResponse::error(10001, $firstMessage, ['errors' => $e->errors()], 422);
        });
