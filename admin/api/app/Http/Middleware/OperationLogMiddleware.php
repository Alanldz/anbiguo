 ...
 *   Route::put('configs/{id}', [...])->middleware('op.log:config,update');
 *
 * 设计取舍：
 *   - 写日志失败**绝不能影响主业务**，全程 try/catch 并只记 warning
...
class OperationLogMiddleware
{
    ...
    private const SENSITIVE_KEYS = [
        ...
        'token', 'authorization', 'cert', 'certificate', 'value',
    ];
    ...
    private function write(Request $request, string $module, string $action, Response $response): void
    {
        ...
        SysLog::create([
            ...
            'before_json' => null,
            'after_json'  => json_encode($this->sanitize($request->except(['password', 'password_confirmation'])), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
